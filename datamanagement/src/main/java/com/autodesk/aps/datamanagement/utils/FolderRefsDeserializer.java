...
public class FolderRefsDeserializer extends JsonDeserializer<FolderRefs> {
    ...
    @Override
    public FolderRefs deserialize(JsonParser jp, DeserializationContext ctxt) throws IOException {
        ObjectMapper mapper = (ObjectMapper) jp.getCodec();
        ObjectNode root = mapper.readTree(jp);

        FolderRefs refs = new FolderRefs();
        ...
        if (root.has("data")) {
            refs.setData(CLEAN_MAPPER.readValue(root.get("data").toString(),
                    new TypeReference<List<FolderRefsData>>() {
                    }));
        }
